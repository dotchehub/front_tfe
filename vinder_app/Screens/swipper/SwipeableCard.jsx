...
const SwipeableCard = ({ item, removeCard, swipedDirection }) => {
  ...
  const [rotateCardTop,setRotateCardTop] = useState(xPosition.interpolate({
    inputRange: [-200, 0, 200],
    outputRange: ['-20deg', '0deg', '20deg'],
  }));
  const [rotateCardBottom,setrotateCardBottom] = useState(xPosition.interpolate({
    inputRange: [-200, 0, 200],
    outputRange: ['20deg', '0deg', '-20deg'],
  }));

  const setTopRotationCardToZero = () => {
    setRotateCardTop(xPosition.interpolate({
      inputRange: [-200, 0, 200],
      outputRange: ['-10deg', '0deg', '10deg'],
    }))
  }
    const setBottomRotationCardToZero = () => {
      setrotateCardBottom(xPosition.interpolate({
        inputRange: [-200, 0, 200],
        outputRange: ['10deg', '0deg', '-10deg'],
      }))
  }
    ...
    return (
      <Animated.View
        ...
        style={[
          styles.cardStyle,
          {
            transform: [{ translateX: xPosition }, { rotate: isUpper?rotateCardTop:rotateCardBottom }, {translateY:yPosition}]
          },
        ]}
        >
          <View style={{ zIndex:100, position:"absolute",top:0,right:0,left:0,bottom:0, flexDirection:"row",justifyContent:"center"}}>
          {[...Array(nbOfPicture)].map((_, i) =>
          <ActualPicture key={i} setOpacity={i==counter?1:0.3}/>
          )}

          </View>
          <Image style={styles.cardStyle} source={item.images[counter].uri}>
          </Image>
          ...
          <Image style={{height:20,width:15,marginRight:10}}source={require("../../images/localisation_logo.png")}></Image>
          ...
          <TouchableOpacity style={!dislike?styles.btn_dislike_not_clicked:styles.btn_dislike_clicked}
          onPress={() => {
              setDislike(true)
              setTopRotationCardToZero()
              setIsUpper(true);
            ...
            <Ionicons name="close" style={{padding:2}} size={43} color={!dislike?"#e2504f":"white"} />
          </TouchableOpacity>
          <TouchableOpacity style={
            !like? styles.btn_like_not_clicked:styles.btn_like_clicked}
          onPress={() => {
            if (!dislike){
            setlike(true)
            setTopRotationCardToZero()
          ...
          <MaterialCommunityIcons style={{padding:10}} name="heart" size={30} color={!like?"#68e674":"white"} />
          </TouchableOpacity>
        </View>
          <Animated.View
        style={{
  ...
  const styles = StyleSheet.create({
    profile_name:{
      paddingLeft:20,
      position:"absolute",top:0,bottom:100,right:0,left:0,
